...
function App() {
  ...
  function currentLocation(){
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        url = `http://api.weatherapi.com/v1/current.json?key=${import.meta.env.VITE_API_KEY}&q=${lat},${long}&aqi=yes`;
        fetch(url)
        .then(res => res.json())
        .then(data => {
          setPlace(data.location.name);
  ...
  function searchCity(){
    ...
    setPlace(city);
    url  = `http://api.weatherapi.com/v1/current.json?key=${import.meta.env.VITE_API_KEY}&q=${city}&aqi=yes`;
    fetch(url)
    .then(res => res.json())
    .then(data => {
      setPlace(data.location.name);
